...
def check_loss_shapes(
    ...
    # 校验对象: VISReg的CLS来源——必须来自骨干最终完整序列，不能误用Predictor或Patch均值。
    ...
    # 校验对象: VISReg跨视图目标——完整Teacher CLS必须和残缺Student CLS逐样本同形。
    if teacher_cls.shape != backbone_features[:, cls_index].shape:
        raise ValueError("VISReg的Student与Teacher CLS必须同形")
